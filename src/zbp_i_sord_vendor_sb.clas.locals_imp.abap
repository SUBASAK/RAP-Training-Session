...
*** Declaration
        TYPES: BEGIN OF ty_buffer_ord.
                INCLUDE TYPE zsord_vendor_SB AS data.
...
ls_failed-%msg = new_message( id = 'ZRAP_SB'
number = 000
...
SELECT MAX( sord ) FROM zsord_vendor_SB INTO @DATA(lv_max_sord).
...
SELECT SINGLE * FROM zsord_vendor_SB WHERE sord = @<lfs_new_data>-Sord
...
SELECT * FROM zsord_vendor_SB WHERE sord = @<lfs_keys>-Sord INTO TABLE @DATA(lt_result_tmp) .
...
LOOP AT keys ASSIGNING FIELD-SYMBOL(<lfs_keys>).
SELECT SINGLE * FROM zsord_vendor_SB WHERE sord = @<lfs_keys>-Sord INTO @DATA(ls_result_tmp) .
...
ENDMETHOD.
ENDCLASS.
CLASS lsc_ZI_SORD_VENDOR_XXX DEFINITION INHERITING FROM cl_abap_behavior_saver.
PROTECTED SECTION.
METHODS check_before_save REDEFINITION.
METHODS finalize REDEFINITION.
METHODS save REDEFINITION.
ENDCLASS.
CLASS lsc_ZI_SORD_VENDOR_XXX IMPLEMENTATION.
